...
def make_streamline(func_3d,start, dL=0.1,
    ...
    while out_of_bounds == False:
        ...
        out_of_bounds_gate, out_of_bounds_wall = bool_out_of_bounds(_r_next, active_boundaries, boundary_type)
...
#             print('r after:', _r_next)
        _counter+=1
        assert ~(np.abs(_r - _r_next) < 0.00001).all(), 'The electron is stuck. Either collected in the anode (Yay!) or on a numerical loop (Nay!)'
